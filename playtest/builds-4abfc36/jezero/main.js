...
function drawMap(c=$('#map'),mini=false){
 ...
 if(journey&&state.mode!=='trial'){
  ...
  if(expeditionRecord?.status==='active'&&level>=2){const pending=nextLookout(journey,expeditionRecord),p=pending&&expeditionRecord.next>=pending.checkpoint?pending:journey.checkpoints[expeditionRecord.next]??journey.finish,q=mapPoint(p,w,h,bounds);ctx.strokeStyle='#9be3ed';ctx.lineWidth=2;ctx.beginPath();ctx.arc(q.x,q.y,6,0,Math.PI*2);ctx.stroke();}ctx.restore();
 }
...
try{
 ...
 area=await loadArea();journey=attachPhotoTour(await loadJourney(),discoveries);
 ...
 $('#welcome p').textContent=`From touchdown to the western frontier. Retrace the route, collect ${journey.lookouts.length} photo lookouts in order. Complete one activity. No score threshold. Expedition kit equipped.`;
