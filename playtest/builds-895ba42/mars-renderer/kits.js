...
export function explorationKit(id,mods={}){const k=raceKit(id,id==='perseverance'&&!!mods.kit);if(id==='perseverance'&&mods.kit)Object.assign(k,{name:'Perseverance · ASTRA clone',speed:KITS.perseverance.speed*2.5,boostSpeed:KITS.perseverance.speed*2.5,acceleration:raceKit(id,true).acceleration,handling:1.15,cloneDrive:true,airYaw:.95,airBrake:600});return k;}
export function ratings(id,{tuned=false,powered=false,boostScale=false,clone=false}={}){const k=clone?explorationKit(id,{kit:powered}):tuned?raceKit(id,powered):kit(id);return [['Top speed',k.speed/(clone&&powered?332.5:boostScale?332.5:133)],['Acceleration',k.acceleration/(clone&&powered?raceKit(id,true).acceleration:boostScale?raceKit('sojourner',true).acceleration:130)],['Handling',tuned?k.handling:1],['Weight',tuned?k.massFactor/1.3:Math.max(.04,MASS[id]/1025)]];}
