...
import {mapBounds,raceSite,mapTarget,advanceEntry,CLOSE_RANGE,regionAt,reachableTarget,worldUnlocked} from './navigation.js';
import {loadArea,discoveries,SITE_VERSION} from './area.js';
import {regions,detailIndices,regionIdForDetail,coreRegionIds,coreDetailIndices,revealFloorRegions} from './regions.js';
import {floorMapBounds} from './floor.js';
...
function syncFloor(){
 if(!area||area.extension.unlocked||!worldUnlocked(progress,coreDetailIndices,coreRegionIds))return;
 revealFloorRegions();area.unlockFloor();Object.assign(mapBounds,floorMapBounds);
 storm.southBoundary=area.bounds.maxY+1600;
 if(ready)notify('Landing plain and Séítah unlocked · Field Notes can guide you there.',10);
 relief.src=new URL('./assets/floor-relief.webp',import.meta.url).href;relief.onload=()=>{dirty=true;};
}
function readSave(){try{const s=JSON.parse(localStorage.getItem(KEY)||'null');if(!s||s.version!==SITE_VERSION)return;
 ...
 if(s.navigation?.target){const t=s.navigation.target;if(Number.isFinite(t.x)&&Number.isFinite(t.y)&&t.x>=0&&t.x<=mapBounds.width&&t.y>=0&&t.y<=mapBounds.maxY)nav.target=reachableTarget(t,area.bounds);}
...
function openDialog(id,caller=null){closeDialogs();backTo=caller;setPause(true);$(id).showModal();if(id==='#pause-dialog')$('#resume').textContent='RESUME';if(id==='#map-dialog'){$('#site-labels').checked=nav.labels;drawMap();}if(id==='#notes-dialog')renderNotes();if(id==='#jump-dialog')renderJumpSites();syncMenuTabs();}
...
function setMapTarget(x,y,w,h){const target=mapTarget(x,y,w,h,area.bounds),old=nav.target;const same=old&&Math.hypot((old.x-target.x)/mapBounds.width*w,(old.y-target.y)/mapBounds.maxY*h)<12;nav.target=same||Math.hypot(target.x-state.x,target.y-state.y)<=CLOSE_RANGE?null:target;save();drawMap();drawMap($('#mini-map'),true);dirty=true;}
...
function renderNotes(){
 syncFloor();
 const sites=detailIndices.filter(i=>progress.collected.includes(i)).length,raceDone=progress.completedActivities.includes('delta-trial'),coreComplete=worldUnlocked(progress,coreDetailIndices,coreRegionIds),complete=worldUnlocked(progress,detailIndices,regions.map(r=>r.regionId??r.id));$('#notes-progress').textContent=complete?'Three Forks, landing plain and Séítah complete · All current discoveries and the Delta circuit explored.':coreComplete?`Three Forks complete · Landing plain and Séítah unlocked · ${progress.regions.length-4} / 2 new regions · ${[7,8].filter(i=>progress.collected.includes(i)).length} / 2 new discoveries`:`${progress.regions.length} / 4 regions · ${sites} / 6 discoveries · Delta circuit: ${raceDone?'complete':'finish a lap'} · Finish these to open the landing plain.`;
 let next=$('#floor-target');if(!next){next=document.createElement('button');next.id='floor-target';next.textContent='Target the landing plain';next.onclick=()=>targetSite(regions.find(r=>r.id==='landing-plain'));$('#notes-progress').after(next);}next.hidden=!coreComplete||complete;
 ...
 $('#notes').replaceChildren(...regions.map(region=>{const id=region.regionId??region.id,known=progress.regions.includes(id),section=document.createElement('section');section.className='note-region';const head=document.createElement('div');head.className='note-region-head';const title=document.createElement('h3');title.textContent=known?region.name:'???';const count=document.createElement('small'),children=detailIndices.filter(i=>regionIdForDetail(i)===id);count.textContent=known?(children.length?`${children.filter(i=>progress.collected.includes(i)).length} / ${children.length} sites`:'REGION OVERVIEW'):'REGION';head.append(title,count);section.append(head,noteRow(region,known));if(known)for(const i of children)section.append(noteRow(discoveries[i],progress.collected.includes(i)));return section;}));
}
...
function mapPoint(p,w,h,b=mapBounds){return {x:(p.x-b.minX)/(b.width-b.minX)*w,y:(p.y-b.minY)/(b.maxY-b.minY)*h};}
function drawMap(c=$('#map'),mini=false){
 const rect=c.getBoundingClientRect(),w=Math.max(1,rect.width),h=Math.max(1,rect.height),dpr=Math.min(2,devicePixelRatio),bounds=mini&&state.mode==='trial'?raceMapBounds():mapBounds;c.width=w*dpr;c.height=h*dpr;const ctx=c.getContext('2d');ctx.scale(dpr,dpr);ctx.fillStyle='#8a6346';ctx.fillRect(0,0,w,h);if(relief.complete&&relief.naturalWidth){const sx=bounds.minX/mapBounds.width*relief.naturalWidth,sy=bounds.minY/mapBounds.maxY*relief.naturalHeight,sw=(bounds.width-bounds.minX)/mapBounds.width*relief.naturalWidth,sh=(bounds.maxY-bounds.minY)/mapBounds.maxY*relief.naturalHeight;ctx.drawImage(relief,sx,sy,sw,sh,0,0,w,h);}ctx.fillStyle='#1e242233';ctx.fillRect(0,0,w,h);
 const occupied=[];
 ...
 for(const p of regions){const id=p.regionId??p.id,q=mapPoint(p,w,h,bounds),known=progress.regions.includes(id);if(q.x<-12||q.x>w+12||q.y<-12||q.y>h+12)continue;ctx.fillStyle=known?'#d5edc1':'#efe2bc';ctx.beginPath();ctx.arc(q.x,q.y,mini?3.4:5,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#17322e';ctx.lineWidth=1;ctx.stroke();ctx.font=`${mini?8:10}px system-ui`;ctx.fillStyle='#f5ead1';mapLabel(known?p.name:'???',q,mini?12:15);}
 for(const i of detailIndices){const p=discoveries[i],id=regionIdForDetail(i);if(!progress.regions.includes(id))continue;const q=mapPoint(p,w,h,bounds);if(q.x<-8||q.x>w+8||q.y<-8||q.y>h+8)continue;ctx.fillStyle=siteColor(p);ctx.globalAlpha=mini?.9:1;if(p.kind==='history')ctx.fillRect(q.x-(mini?3:4),q.y-(mini?3:4),mini?6:8,mini?6:8);else{ctx.beginPath();ctx.arc(q.x,q.y,mini?2.8:4,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=1;if(!mini&&nav.labels){ctx.font=`${w<260?8:10}px system-ui`;ctx.textAlign='center';mapLabel(progress.collected.includes(i)?(p.mapName??p.name):'???',q,12);}}
...
function step(dt){previous=capturePose(state,area.ground(state.x,state.y));driveStep(state,input,dt,area.course,{...area.driveArea,roverHandling:true,airControl:state.tune.boostKit,boostKit:state.tune.boostKit});if(stormsEnabled)storm.tick(dt,state,()=>gpu.tracks.reset());
 for(const p of regions){const id=p.regionId??p.id,key='region:'+id,distance=Math.hypot(state.x-p.x,state.y-p.y);if(distance>CLOSE_RANGE)observedInside.delete(key);if(state.mode!=='trial'&&!state.air&&!state.turnaround&&distance<8&&!observedInside.has(key)){observedInside.add(key);if(!progress.regions.includes(id))progress.regions.push(id);state.regions=[...progress.regions];state.v=state.vz=state.boost=0;previous=capturePose(state,area.ground(state.x,state.y));save();updateHud();showCard(p,null,'region');return;}}
 // Each ground-level entry opens the observation, including return visits.
 for(const i of detailIndices){const p=discoveries[i],distance=Math.hypot(state.x-p.x,state.y-p.y);if(distance>CLOSE_RANGE)observedInside.delete(i);if(!progress.regions.includes(regionIdForDetail(i)))continue;if(state.mode!=='trial'&&!state.air&&!state.turnaround&&distance<8&&!observedInside.has(i)){observedInside.add(i);if(!state.collected.includes(i))state.collected.push(i);progress.collected=[...state.collected];state.v=0;state.vz=0;state.boost=0;if(p.focus){state.heading=Math.atan2(p.focus.x-state.x,-(p.focus.y-state.y));gpu.camera={x:state.x,y:state.y,h:state.heading};gpu.bodySlope=null;}previous=capturePose(state,area.ground(state.x,state.y));save();updateHud();showDiscovery(i);break;}}
...
function frame(t){const dt=Math.min(.05,(t-last)/1000||0);last=t;if(ready){if(!paused){acc+=dt;while(acc>=1/60&&!paused){step(1/60);acc-=1/60;}if(t-lastDraw>=(quality==='performance'?32:0)){draw(Math.min(.05,(t-lastDraw)/1000));lastDraw=t;}if(t-lastSave>3000){save();lastSave=t;}}else if(dirty){draw(0);dirty=false;}
 if(t-lastHud>100){updateHud();lastHud=t;}if(t-lastMap>200){if(minimap.state.enabled)drawMap($('#mini-map'),true);if($('#map-dialog').open)drawMap();lastMap=t;}}
 requestAnimationFrame(frame);}
$('#discovery-continue').onclick=closeSheet;
$('#explore').onclick=explore;$('#race-again').onclick=chooseRace;$('#result-explore').onclick=explore;
...
function renderJumpSites(){const list=$('#jump-sites');list.replaceChildren();const add=(p,prefix='')=>{const b=document.createElement('button');b.textContent=prefix+p.name;b.onclick=()=>requestJump(p);list.append(b);};for(const region of regions){const id=region.regionId??region.id;if(progress.regions.includes(id))add(region);}for(const i of detailIndices)if(progress.collected.includes(i))add(discoveries[i],'SITE · ');$('#jump-empty').hidden=!!list.children.length;syncMenuTabs();}
...
try{
 ...
 window.__jezero={get state(){return state},area,gpu,storm,progress,get worldUnlocked(){return worldUnlocked(progress,coreDetailIndices,coreRegionIds)},get expansionComplete(){return area.extension.unlocked&&worldUnlocked(progress,detailIndices,regions.map(r=>r.regionId??r.id))},get paused(){return paused},get input(){return input},landmarks,regions,nav,raceSite,get entryTime(){return entryTime},mods,minimap,get stormsEnabled(){return stormsEnabled},resetAt,step,draw,startLap,chooseRace,explore,setPause,save,openDialog,renderNotes,drawMap};requestAnimationFrame(frame);
