...
let currentRegion='';
function specialMode(){if(state.air&&!state.artHop)return 'Slow';if(expedition?.active?.id==='art')return 'Hop';if(!state.air&&(reverseHeld||state.impact>0||area.rocks.some(([x,y,r])=>Math.hypot(x-state.x,y-state.y)<r+18&&(x-state.x)*Math.sin(state.heading)-(y-state.y)*Math.cos(state.heading)>0)))return 'Reverse';return state.mode==='trial'?'Boost':'Camera';}
...
function drawMap(c=$('#map'),mini=false){
 ...
 if(area.world&&state.mode!=='trial'){ctx.save();ctx.beginPath();ctx.rect(frame.x,frame.y,frame.width,frame.height);ctx.clip();const course=journey.course,on=expeditionRecord?.status==='active',segments=on?[{points:course.points}]:routeDisplay;ctx.strokeStyle='#ffe59dcc';ctx.lineWidth=on?Math.max(1,course.halfWidth*2*frame.width/(bounds.width-bounds.minX)):mini?.8:1.1;ctx.lineJoin='round';ctx.lineCap='round';ctx.beginPath();for(const segment of segments)segment.points.forEach((p,i)=>{const q=mapPoint(p,w,h,bounds);i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y);});ctx.stroke();if(on){let last=null;ctx.strokeStyle='#493e2c';ctx.lineWidth=1.4;for(let i=1;i<course.points.length;i++){const q=mapPoint(course.points[i],w,h,bounds);if(last&&Math.hypot(q.x-last.x,q.y-last.y)<(mini?22:32))continue;const a=mapPoint(course.points[i-1],w,h,bounds),angle=Math.atan2(q.y-a.y,q.x-a.x);ctx.save();ctx.translate(q.x,q.y);ctx.rotate(angle);ctx.beginPath();ctx.moveTo(-4,-3);ctx.lineTo(0,0);ctx.lineTo(-4,3);ctx.stroke();ctx.restore();last=q;}for(const [i,f]of course.facts.entries()){const q=mapPoint(course.points[f.routeIndex],w,h,bounds);ctx.fillStyle=i<expeditionRecord.run.facts.length?'#a6dec1':'#ffe6a4';ctx.beginPath();ctx.arc(q.x,q.y,mini?3:5,0,Math.PI*2);ctx.fill();}}ctx.restore();}
