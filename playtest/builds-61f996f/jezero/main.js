import {viewBounds,zoomView,panView,smoothRoute,activitySymbols} from './map-view.js';
...
const RACE_VERSION='straight-cruise-v6';
...
let routeDisplay=[];const mapView={zoom:1,x:0,y:0};
...
function finishLap(){const time=state.time,old=progress.bests[selected];if(!old||time<old)progress.bests[selected]=time;if(!progress.completedActivities.includes(raceSite.id))progress.completedActivities.push(raceSite.id);
...
function drawMap(c=$('#map'),mini=false){
 ...
 for(const site of expedition?.sites??[]){const anchor=mapPoint(site,w,h,bounds),q={...anchor};if(site.id==='long-jump')q.x-=mini?8:12;if(site.id==='target-jump')q.x+=mini?8:12;ctx.fillStyle='#173532';ctx.strokeStyle='#f4d997';ctx.lineWidth=1;ctx.beginPath();ctx.arc(q.x,q.y,mini?7:10,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.font=`${mini?11:15}px system-ui`;ctx.textAlign='center';ctx.fillStyle='#fff0cb';ctx.fillText(activitySymbols[site.id],q.x,q.y+4);}ctx.restore();
...
function wayfinding(){
 ...
 if(state.mode==='free'&&!expedition?.photo){ctx.strokeStyle='#f5dfae80';ctx.lineWidth=1;ctx.beginPath();for(const segment of routeDisplay){let pen=false;for(const p of segment.points){if(Math.hypot(p.x-state.x,p.y-state.y)>1000){pen=false;continue;}const q=gpu.project(p.x,p.y,area.ground(p.x,p.y)+.6);if(q.depth<8||q.x<0||q.x>w||q.y<0||q.y>h){pen=false;continue;}pen?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y);pen=true;}}ctx.stroke();}
...
function targetSite(p){nav.target=reachableTarget(p,area.bounds);save();dirty=true;$('#jump-feedback').textContent=`Target set · ${p.name}`;notify(`Target set · ${p.name}`,3);}
...
function sitePreview(p){
 const c=document.createElement('canvas');c.className='site-preview';c.width=160;c.height=100;c.setAttribute('role','img');c.setAttribute('aria-label',p.name+' location on Jezero map');const ctx=c.getContext('2d'),image=satellite.naturalWidth?satellite:relief,f=mapFrame(160,100);ctx.fillStyle='#34423b';ctx.fillRect(0,0,160,100);if(image.naturalWidth)ctx.drawImage(image,f.x,f.y,f.width,f.height);const q=mapPoint(p,160,100);ctx.strokeStyle='#fff3ac';ctx.lineWidth=2;ctx.beginPath();ctx.arc(q.x,q.y,6,0,Math.PI*2);ctx.stroke();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(q.x,q.y,2,0,Math.PI*2);ctx.fill();return c;
}
