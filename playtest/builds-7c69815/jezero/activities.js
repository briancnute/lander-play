...
export function createActivities(api,journey,record,stored={}){
 ...
 $('#journey-start').onclick=()=>confirm('Start a new expedition?',`Follow the wide course and visit every checkpoint in order. Your score is the percentage of moving time spent on the path. Photographs and activities are optional. The existing 2.5x kit is equipped for this expedition. Begin at Octavia E. Butler Landing; only this expedition resets, not discoveries, photographs or activity records.`,start,'Start expedition');
 ...
 function complete(copy){if(!active)return;const id=active.id;results[id]={date:new Date().toISOString(),summary:copy};const qualifies=id!=='neretva-jump'||boostStage===3;if(qualifies)creditActivity(record,id);api.save();api.openDialog('#activity-result');$('#activity-result-title').textContent=active.name;$('#activity-result-copy').textContent=copy+(record.status==='active'&&qualifies?(routeRun?' · Optional activity recorded.':' · Required expedition activity complete.'):'');}
 ...
 function finishRouteRun(){record.status='finished';record.finishReached=true;record.next=journey.checkpoints.length;if(record.freeRoamSetup){api.restoreTravel(record.freeRoamSetup);api.applyTravel();record.freeRoamSetup=null;}record.returnPoint=null;returnPose=null;api.nav.target=null;api.save();api.openDialog('#journey-finish');$('#journey-finish-copy').textContent=`Every checkpoint reached. ${routeScore(record.run)}% of your driving time was inside the course.${record.run.upgraded?' Score covers driving since the route upgrade.':''} Your photographs, discoveries and optional activities remain in your field records.`;}
 function update(){
  ...
  if(routeRun){routeRun.update();$('#journey-hud').textContent=routeRun.summary();}
