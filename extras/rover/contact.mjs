...
export function moveWithContact(x,y,dx,dy,mesa,rocks){const steps=Math.max(1,Math.ceil(Math.hypot(dx,dy)/2));let hit=false;
 for(let step=0;step<steps;step++){x+=dx/steps;y+=dy/steps;
  for(let pass=0;pass<3;pass++){
   for(const [rx,ry,r]of rocks){const vx=x-rx,vy=y-ry,d=Math.hypot(vx,vy),min=r+ROVER_RADIUS;if(d<min){hit=true;const nx=d?vx/d:1,ny=d?vy/d:0;x=rx+nx*(min+.001);y=ry+ny*(min+.001);}}
   ...
   const within=inside(x,y,mesa);if(within||best<ROVER_RADIUS){hit=true;const c=closest;let nx=best?(x-c.px)/best:c.vy/Math.hypot(c.vx,c.vy),ny=best?(y-c.py)/best:-c.vx/Math.hypot(c.vx,c.vy);if(within){nx=-nx;ny=-ny;}x=c.px+nx*(ROVER_RADIUS+.001);y=c.py+ny*(ROVER_RADIUS+.001);}
  }
 }return {x,y,hit};
}
