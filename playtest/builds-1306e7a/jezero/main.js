import {checkpointPlayback} from './checkpoint-playback.js';
import {createRouteGuidance,routeWindow} from './route-guidance.js';
import {buildRouteCourse} from './route-run.js';
...
import {attachPhotoTour,timeTour,nextLookout} from './photo-tour.js';
...
const RACE_VERSION='neretva-rally-v9';
...
const mods={kit:false};let driftEnabled=false,stormsEnabled=true,raceExplorer=null;
let checkpointResumeAge=3;
let raceId='delta-trial',specialWasHeld=false,reverseHeld=false,brakeHeld=false;
...
function readSave(){try{const s=JSON.parse(localStorage.getItem(KEY)||'null');if(!s||s.version!==SITE_VERSION)return;
 ...
 if(s.raceVersion===RACE_VERSION&&s.bests&&typeof s.bests==='object')for(const course of ['delta-trial','landing-trial'])for(const rover of Object.keys(KITS))for(const suffix of ['',':drift']){const id=course+':'+rover+suffix;if(Number.isFinite(s.bests[id])&&s.bests[id]>0)progress.bests[id]=s.bests[id];}
 ...
 driftEnabled=s.driftEnabled===true;mods.kit=s.mods?.kit===true||s.mods?.speed===true||s.mods?.air===true;stormsEnabled=s.stormsEnabled!==false;minimap.restore(s.minimap);nav.labels=s.navigation?.labels===true;
...
function save(){if(!ready)return;syncFloor();const position=state.mode==='free'&&!state.air&&!state.turnaround?{x:state.x,y:state.y,heading:state.heading}:progress.position;
 try{localStorage.setItem(KEY,JSON.stringify({version:SITE_VERSION,worldScale:WORLD_SCALE,siteCatalog:3,collected:progress.collected,regions:progress.regions,completedActivities:progress.completedActivities,bests:progress.bests,legacyBests:progress.legacyBests,raceVersion:RACE_VERSION,rover:raceExplorer??selected,position,quality,light,seenLandmarks:progress.seenLandmarks,navigation:nav,mods,driftEnabled,stormsEnabled,minimap:minimap.state,expedition:expeditionRecord,activityState:expedition?.snapshot()??activitySave,basemap,mapStyleVersion:2}));progress.position=position;}catch{if(!saveWarned){saveWarned=true;notify('Progress cannot be saved in this browser.');}}}
function notify(text,seconds=5){state.message=text;state.messageUntil=state.t+seconds;dirty=true;}
function collectTourPhoto(i){
 const p=discoveries[i];observedInside.add(i);
 if(!progress.collected.includes(i))progress.collected.push(i);
 state.collected=[...progress.collected];state.v=state.vz=state.boost=0;
 if(p.focus){state.heading=Math.atan2(p.focus.x-state.x,-(p.focus.y-state.y));gpu.camera={x:state.x,y:state.y,h:state.heading};gpu.bodySlope=null;}
 previous=capturePose(state,area.ground(state.x,state.y));updateHud();showDiscovery(i);
}
...
function resetAt(p=area.start,mode='free'){
 ...
 state.raceBoosts=mode==='trial'?3:0;specialWasHeld=false;reverseHeld=false;brakeHeld=false;
}
...
function openDialog(id,caller=null){if(id==='#notes-dialog')id='#jump-dialog';if(expedition?.photo)expedition.closeCamera();closeDialogs();backTo=caller;setPause(true);$(id).showModal();if(id==='#pause-dialog'){$('#resume').textContent='RESUME';$('#drift-enabled').disabled=state?.mode==='trial';$('#drift-enabled').title=state?.mode==='trial'?'Finish or leave the race before changing steering':'';}if(id==='#map-dialog'){$('#site-labels').checked=nav.labels;drawMap();}if(id==='#notes-dialog')renderNotes();if(id==='#jump-dialog')renderJumpSites();syncMenuTabs();}
...
function finishLap(){const time=state.time,key=raceId+':'+selected+(driftEnabled?':drift':''),old=progress.bests[key];if(!old||time<old)progress.bests[key]=time;if(!progress.completedActivities.includes(raceId))progress.completedActivities.push(raceId);
 ...
 $('#result-time').textContent=clock(time);$('#result-best').textContent=(!old||time<old?'Your best in this rover':`Best · ${clock(old)}`)+(driftEnabled?' · Drift steering':'');
 state.mode='free';state.done=false;state.countdown=0;state.v=0;openDialog('#result-dialog');updateHud();save();}
let currentRegion='';
function specialMode(){if(state.air&&!state.artHop)return 'Slow';if(expedition?.active?.id==='art')return 'Hop';if(brakeHeld)return 'Brake';if(!state.air&&(reverseHeld||(Math.abs(state.v)<1&&(state.impact>0||area.rocks.some(([x,y,r])=>Math.hypot(x-state.x,y-state.y)<r+18&&(x-state.x)*Math.sin(state.heading)-(y-state.y)*Math.cos(state.heading)>0)))))return 'Reverse';return state.mode==='trial'?'Boost':Math.abs(state.v)>=1?'Brake':'Camera';}
...
const mapCanvas=$('#map');
attachMapGestures(mapCanvas,{world:mapBounds,view:mapView,draw:()=>drawMap(),target:setMapTarget});
...
function drawMap(c=$('#map'),mini=false){
 ...
 if(area.world&&state.mode!=='trial'){
  ctx.save();ctx.beginPath();ctx.rect(frame.x,frame.y,frame.width,frame.height);ctx.clip();
  const course=journey.course,on=expeditionRecord?.status==='active'&&!expeditionRecord.returnPoint;
  const stroke=(points,color,width)=>{ctx.strokeStyle=color;ctx.lineWidth=width;ctx.lineJoin=ctx.lineCap='round';ctx.beginPath();points.forEach((p,i)=>{const q=mapPoint(p,w,h,bounds);i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y);});ctx.stroke();};
  if(on){
   // Overview retains historical context quietly; the minimap shows only the current passage.
   if(!mini)stroke(course.points,'#ffe59d45',1);
   const active=routeWindow(course,expeditionRecord.run);
   stroke(active.points,'#173d3a',mini?6:7);stroke(active.points,'#73fff0',mini?3:4);
   let last=null;ctx.strokeStyle='#ffffff';ctx.lineWidth=1.8;
   for(let i=1;i<active.points.length;i++){const q=mapPoint(active.points[i],w,h,bounds);if(last&&Math.hypot(q.x-last.x,q.y-last.y)<24)continue;const a=mapPoint(active.points[i-1],w,h,bounds);ctx.save();ctx.translate(q.x,q.y);ctx.rotate(Math.atan2(q.y-a.y,q.x-a.x));ctx.beginPath();ctx.moveTo(-5,-4);ctx.lineTo(0,0);ctx.lineTo(-5,4);ctx.stroke();ctx.restore();last=q;}
   const target=mapPoint(active.target,w,h,bounds);ctx.strokeStyle='#fff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(target.x,target.y,5,0,Math.PI*2);ctx.stroke();
  }else for(const segment of routeDisplay)stroke(segment.points,'#ffe59dcc',mini?.8:1.1);
  ctx.restore();
 }
 if(journey&&state.mode!=='trial'&&expeditionRecord?.status!=='active'){
  ...
  for(const [name,p,color]of [['START',journey.course.points[0],'#b7efc3'],['FINISH',journey.course.points.at(-1),'#fff0ba']]){const q=mapPoint(p,w,h,bounds);ctx.fillStyle=color;ctx.fillRect(q.x-3,q.y-3,6,6);if(!mini&&level>0){ctx.font='bold 10px system-ui';ctx.textAlign='center';ctx.fillText(name,q.x,q.y+25);const tw=ctx.measureText(name).width;occupied.push({x:q.x-tw/2-3,y:q.y+14,w:tw+6,h:15});}}
  if(expeditionRecord?.status==='active'&&level>=2){const pending=nextLookout(journey,expeditionRecord),p=expeditionRecord.run?journey.course.points[Math.min(expeditionRecord.run.next,journey.course.points.length-1)]:pending&&expeditionRecord.next>=pending.checkpoint?pending:journey.checkpoints[expeditionRecord.next]??journey.finish,q=mapPoint(p,w,h,bounds);ctx.strokeStyle='#9be3ed';ctx.lineWidth=2;ctx.beginPath();ctx.arc(q.x,q.y,6,0,Math.PI*2);ctx.stroke();}ctx.restore();
 }
 ...
 if(state.mode==='trial'){ctx.strokeStyle='#ddb17a';ctx.lineWidth=mini?9:12;ctx.lineJoin='round';ctx.beginPath();area.course.route.forEach((p,i)=>{const q=mapPoint(p,w,h,bounds);i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y);});if(!area.course.start)ctx.closePath();ctx.stroke();for(const p of [...area.course.gates,area.course.finish]){const q=mapPoint(p,w,h,bounds);ctx.beginPath();ctx.arc(q.x,q.y,mini?2.5:3.5,0,Math.PI*2);ctx.fillStyle='#e6f0c8';ctx.fill();}}
 ctx.save();ctx.beginPath();ctx.rect(frame.x,frame.y,frame.width,frame.height);ctx.clip();
 const games=[];
 for(const site of expedition?.mapSites??[]){const q=mapPoint(site,w,h,bounds);if(site.id==='long-jump')q.x-=6;if(site.id==='target-jump')q.x+=6;if(q.x<frame.x||q.x>frame.x+frame.width||q.y<frame.y||q.y>frame.y+frame.height)continue;ctx.fillStyle='#ffe0a0';ctx.strokeStyle='#34372d';ctx.lineWidth=1;ctx.beginPath();for(let i=0;i<10;i++){const a=-Math.PI/2+i*Math.PI/5,r=i%2?2.6:6,x=q.x+Math.cos(a)*r,y=q.y+Math.sin(a)*r;i?ctx.lineTo(x,y):ctx.moveTo(x,y);}ctx.closePath();ctx.fill();ctx.stroke();occupied.push({x:q.x-6,y:q.y-6,w:12,h:12});games.push({site,q});}ctx.restore();
...
function wayfinding(){
 ...
 gpu.expeditionGateCount=0;
 if(gpu.expeditionRouteActive&&state.mode==='free'&&!expedition?.photo){
  const course=journey.course;
  for(let i=expeditionRecord.run.facts.length;i<course.facts.length;i++){
   const f=course.facts[i],p=course.points[f.routeIndex];if(Math.hypot(p.x-state.x,p.y-state.y)>1400)continue;
   const a=course.points[Math.max(0,f.routeIndex-2)],b=course.points[Math.min(course.points.length-1,f.routeIndex+2)],heading=Math.atan2(b.x-a.x,-(b.y-a.y)),z=area.ground(p.x,p.y);
   gpu.mesh(posts,[p.x,p.y,z,heading],7);gpu.expeditionGateCount++;
   const q=gpu.project(p.x,p.y,z+32);if(q.depth>4&&q.x>50&&q.x<w-50&&q.y>160&&q.y<h-125){ctx.font='bold 12px system-ui';ctx.textAlign='center';ctx.fillStyle='#d9ffdb';ctx.fillText(`CHECKPOINT ${i+1}`,q.x,q.y);}
  }
 }

 if(journey&&state.mode==='free'&&!expedition?.photo&&expeditionRecord?.status!=='active')for(const [label,p]of [['START',journey.course.points[0]],['FINISH',journey.course.points.at(-1)]]){if(Math.hypot(p.x-state.x,p.y-state.y)>650)continue;const z=area.ground(p.x,p.y);gpu.mesh(posts,[p.x,p.y,z,0],7);const q=gpu.project(p.x,p.y,z+30);if(q.depth>4&&q.x>30&&q.x<w-30&&q.y>120&&q.y<h-125){ctx.textAlign='center';ctx.font='bold 12px system-ui';ctx.fillStyle='#f5e5bd';ctx.fillText(label,q.x,q.y);}}
...
function draw(dt=0){gpu.expeditionRouteActive=expeditionRecord?.status==='active'&&!expeditionRecord.returnPoint;gpu.quality=quality;gpu.storm=storm;const shown=paused?state:displayState(previous,state,acc*60,area.ground(state.x,state.y));gpu.draw(shown,dt,$('#labels'));sky.draw(gpu,shown);wayfinding();placeLamps();}

function step(dt){previous=capturePose(state,area.ground(state.x,state.y));
 const pressed=!!input.special&&!specialWasHeld;specialWasHeld=!!input.special;if(!input.special){reverseHeld=false;brakeHeld=false;}
 const action=specialMode();if(pressed){if(action==='Brake')brakeHeld=true;else if(action==='Hop')expedition.hop();else if(action==='Boost'&&state.countdown<=0&&state.raceBoosts>0&&state.boost<=0){state.raceBoosts--;state.boost=state.tune.boostSeconds;}else if(action==='Reverse')reverseHeld=true;else if(action==='Camera'&&!state.air&&Math.abs(state.v)<1){expedition.camera();return;}}
 if(!stepArtHop(state,input,dt,area.ground,area.bounds))driveStep(state,{...input,brakeOnly:brakeHeld,brake:input.brake||brakeHeld||reverseHeld||(state.air&&!state.artHop&&input.special)},dt,area.course,{...area.driveArea,...(state.mode==='trial'?{pickups:[]}:{}),cruise:true,arcadeHandling:state.mode==='free'&&!expedition?.active,driftHandling:driftEnabled,surfaceGrip:s=>{const slope=Math.hypot(area.ground(s.x+8,s.y)-area.ground(s.x-8,s.y),area.ground(s.x,s.y+8)-area.ground(s.x,s.y-8))/16;return area.speedMultiplier(s)>1?240:120+100*Math.min(1,slope*7);},roverHandling:true,airControl:state.tune.boostKit,boostKit:state.tune.boostKit});if(stormsEnabled&&!expedition?.protectsArt)storm.tick(dt,state,()=>gpu.tracks.reset());if(expedition?.tick(dt,previous))return;
 if(expedition?.active){if(state.mode==='trial'&&state.done)finishLap();return;}
 if(state.mode==='free'&&expeditionRecord?.status==='active'&&expeditionRecord.run&&!expeditionRecord.returnPoint)return;
 ...
 if(nav.target&&!nav.target.tourLookout&&Math.hypot(state.x-nav.target.x,state.y-nav.target.y)<=CLOSE_RANGE){nav.target=null;save();dirty=true;}
...
function frame(t){const elapsed=(t-last)/1000||0,dt=Math.min(.05,elapsed);last=t;if(ready){if(!document.hidden)timeTour(expeditionRecord,elapsed,expedition?.active?'activities':paused?'menus':'driving');if(!paused){if(!document.hidden)checkpointResumeAge=Math.min(3,checkpointResumeAge+elapsed);const playback=checkpointPlayback(checkpointResumeAge);acc+=dt*playback;while(acc>=1/60&&!paused){step(1/60);acc-=1/60;}if(t-lastDraw>=(quality==='performance'?32:0)){draw(Math.min(.05,(t-lastDraw)/1000)*playback);lastDraw=t;}if(t-lastSave>3000){save();lastSave=t;}}else if(dirty){draw(0);dirty=false;}
...
async function chooseRace(id='delta-trial'){if(expeditionRecord?.run&&expeditionRecord.status==='active')expedition?.remember();raceId=typeof id==='string'&&area.courses[id]?id:'delta-trial';area.course=area.courses[raceId];area.start={...(area.course.start??area.course.finish)};if(gpu.road)gpu.gl.deleteTexture(gpu.road.texture);gpu.road=raceSurface(gpu.gl,area.course.route,58,!area.course.start);entryTime=0;entryArmed=false;closeDialogs();setPause(true);try{await openCraft(selected,id=>{raceExplorer??=selected;selected=id;startLap();},{race:true,jezero:true,compact:true,onCancel:()=>{entryArmed=false;expedition?.stop();restoreExplorer();state.mode='free';state.tune=explorationKit(selected,mods);openDialog('#pause-dialog');}});}catch{openDialog('#pause-dialog');notify('Rover selection could not load. Try again.');}}
...
$('#light').onchange=e=>{light=e.target.value;lighting();for(const b of document.querySelectorAll('[data-light]'))b.setAttribute('aria-pressed',String(b.dataset.light===light));dirty=true;save();};$('#drift-enabled').onchange=e=>{driftEnabled=e.target.checked;state.driftVX=null;state.driftVY=null;state.slip=0;save();};$('#quality').onchange=e=>{quality=e.target.value;dirty=true;save();};$('#storms-enabled').onchange=e=>{stormsEnabled=e.target.checked;if(!stormsEnabled){storm.age=-1;storm.haze=0;storm.wait=75;}dirty=true;save();};$('#map-enabled').onchange=e=>{minimap.state.enabled=e.target.checked;minimap.layout();dirty=true;save();};
...
try{
 // Bind photographic stops before restoring expedition saves.
 area=await loadArea();journey=attachPhotoTour(await loadJourney(),discoveries);journey.course=buildRouteCourse(journey,area.rocks);
 area.world.segments=journey.segments;routeDisplay=smoothRoute(journey.segments);expeditionRecord=restoreExpedition(null,journey);syncFloor();readSave();gpu=new GPURenderer($('#world'),area);gpu.shadowMode='none';gpu.softSurfaceLighting=true;gpu.flightLighting=1;posts=gateGeometry();gpu.routeRibbon=createRouteRibbon(gpu,routeDisplay,area.ground);gpu.expeditionRibbon=createRouteGuidance(gpu,journey.course,()=>expeditionRecord.run,area.ground);storm.southBoundary=area.bounds.maxY+1600;storm.bounds=area.bounds;Object.assign(mapView,{x:(mapBounds.minX+mapBounds.width)/2,y:(mapBounds.minY+mapBounds.maxY)/2});resetAt(progress.position??{...journey.start,heading:-.4});for(const i of progress.collected)if(discoveries[i]&&Math.hypot(state.x-discoveries[i].x,state.y-discoveries[i].y)<CLOSE_RANGE)observedInside.add(i);for(const p of regions){const id=p.regionId??p.id;if(progress.regions.includes(id)&&Math.hypot(state.x-p.x,state.y-p.y)<CLOSE_RANGE)observedInside.add('region:'+id);}entryArmed=Math.hypot(state.x-raceSite.x,state.y-raceSite.y)>=raceSite.radius;ready=true;minimap.layout();$('#map-enabled').checked=minimap.state.enabled;$('#storms-enabled').checked=stormsEnabled;$('#drift-enabled').checked=driftEnabled;$('#quality').value=quality;$('#light').value=light;for(const b of document.querySelectorAll('[data-light]'))b.setAttribute('aria-pressed',String(b.dataset.light===light));$('#loading').hidden=true;
 area.manualRaceBoosts=true;area.courses={'delta-trial':area.course,'landing-trial':landingCircuit()};
 expedition=createActivities({area,gpu,storm,nav,collectLookout:collectTourPhoto,viewLookout:i=>showCard(discoveries[i],'#jump-dialog'),get state(){return state},reset:resetAt,explore,resume:()=>setPause(false),resumeCheckpoint:()=>{checkpointResumeAge=0;setPause(false);},pause:()=>setPause(true),openDialog,closeDialogs,save,draw,sitePreview,targetSite,endLap,chooseRace,travelSetup:()=>({rover:selected,kit:mods.kit}),restoreTravel:p=>{if(KITS[p.rover])selected=p.rover;if(typeof p.kit==='boolean')mods.kit=p.kit;},enableTourKit:()=>{selected='perseverance';mods.kit=true;},applyTravel:()=>{raceExplorer=null;resetAt({x:state.x,y:state.y,heading:state.heading});},enableKit:()=>{selected='perseverance';mods.kit=true;}},journey,expeditionRecord,activitySave);
 const legend=document.createElement('details');legend.className='map-legend';const legendTitle=document.createElement('summary');legendTitle.textContent='Map key';legend.append(legendTitle);const key=document.createElement('div');key.textContent='Cyan arrows: your route now · Faint line: full journey · ○ Next route point · ● Lookouts · ★ Activities · ⌃ Rover';legend.append(key);for(const site of expedition.sites){const item=document.createElement('div');item.textContent='★ '+site.name;legend.append(item);}$('.map-wrap').append(legend);
 $('#welcome p').textContent=`CLASSIFIED · You are piloting a souped-up Perseverance clone. Retrace the recorded journey to restore its memory partition, and complete four roadside systems missions. Follow the chevrons; shortcuts and airtime are allowed. The upgraded clone drive is equipped when you start the mission. Ingenuity is an optional discovery.`;
 if(expeditionRecord.status==='active'&&!nav.target?.activityId)expedition.guide();
 ...
 $('#world').addEventListener('webglcontextlost',e=>{e.preventDefault();setPause(true);expedition?.stop();closeDialogs();setPause(true);ready=false;$('#loading').hidden=false;$('#loading h2').textContent='The graphics paused.';$('#loading p').textContent='Reload to restore your last saved progress.';const b=document.createElement('button');b.textContent='Reload terrain';b.onclick=()=>location.reload();$('#loading').append(b);});
 window.__jezero={get state(){return state},area,gpu,storm,progress,expedition,get worldUnlocked(){return true},get expansionComplete(){return area.extension.unlocked&&detailIndices.every(i=>progress.collected.includes(i))&&regions.filter(r=>!r.ambient).every(r=>progress.regions.includes(r.regionId??r.id))},get playback(){return checkpointPlayback(checkpointResumeAge)},get paused(){return paused},get input(){return input},landmarks,regions,nav,raceSite,mapView,routeDisplay,get entryTime(){return entryTime},mods,minimap,get stormsEnabled(){return stormsEnabled},resetAt,step,draw,startLap,chooseRace,explore,setPause,save,openDialog,renderNotes,drawMap};requestAnimationFrame(frame);
