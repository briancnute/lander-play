...
export function raceKit(id,powered=false){const k=kit(id);k.handling=HANDLING[id]??1;k.massFactor=.7+.6*Math.sqrt((MASS[id]??1025)/1025);k.boostKit=powered;if(powered){k.speed*=5;k.boostSpeed=k.speed;k.acceleration/=k.massFactor;}return k;}
export function explorationKit(id,mods={}){return raceKit(id,id==='perseverance'&&!!mods.kit);}
export function ratings(id,{tuned=false,powered=false,boostScale=false}={}){const k=tuned?raceKit(id,powered):kit(id);return [['Top speed',k.speed/(boostScale?665:133)],['Acceleration',k.acceleration/(boostScale?raceKit('sojourner',true).acceleration:130)],['Handling',tuned?k.handling:1],['Weight',tuned?k.massFactor/1.3:Math.max(.04,MASS[id]/1025)]];}
