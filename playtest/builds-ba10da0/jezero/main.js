...
function drawMap(c=$('#map'),mini=false){
 ...
 const rect=c.getBoundingClientRect(),w=Math.max(1,rect.width),h=Math.max(1,rect.height),dpr=Math.min(2,devicePixelRatio),bounds=mini?(state.mode==='trial'?raceMapBounds():nearby):mapBounds,frame=mapFrame(w,h,bounds);c.width=w*dpr;c.height=h*dpr;const ctx=c.getContext('2d');ctx.scale(dpr,dpr);ctx.fillStyle='#29332f';ctx.fillRect(0,0,w,h);if(relief.complete&&relief.naturalWidth){const spanX=mapBounds.width-mapBounds.minX,spanY=mapBounds.maxY-mapBounds.minY,sx=(bounds.minX-mapBounds.minX)/spanX*relief.naturalWidth,sy=(bounds.minY-mapBounds.minY)/spanY*relief.naturalHeight,sw=(bounds.width-bounds.minX)/spanX*relief.naturalWidth,sh=(bounds.maxY-bounds.minY)/spanY*relief.naturalHeight;ctx.drawImage(relief,sx,sy,sw,sh,frame.x,frame.y,frame.width,frame.height);}ctx.fillStyle='#1e242233';ctx.fillRect(frame.x,frame.y,frame.width,frame.height);
 const occupied=[];
 if(area.world&&state.mode!=='trial'){ctx.save();ctx.beginPath();ctx.rect(frame.x,frame.y,frame.width,frame.height);ctx.clip();ctx.strokeStyle='#f5d28c88';ctx.lineWidth=mini?.6:1;ctx.beginPath();for(const segment of area.world.segments)segment.points.forEach((p,i)=>{const q=mapPoint(p,w,h,bounds);i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y);});ctx.stroke();ctx.restore();}
