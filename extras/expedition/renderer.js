...
export class Renderer {
 ...
 if(this.chunkKey!==`${chunkX},${chunkY}`||!this.mesh){this.chunkKey=`${chunkX},${chunkY}`;const active=new Set();this.mesh=[];
  ...
  this.farMesh=[];for(let j=-24;j<=24;j++)for(let i=-24;i<=24;i++){const x=chunkX*240+i*160,y=chunkY*240+j*160,corners=[[x,y],[x+160,y],[x+160,y+160],[x,y+160]].map(([x,y])=>[x,y,ground(x,y)]);this.farMesh.push({x:x+80,y:y+80,corners,normal:[-(corners[1][2]-corners[0][2])/160,-(corners[3][2]-corners[0][2])/160,1],color:sedimentColor(x+80,y+80),far:true});}
 ...
 const visible=[];for(const tile of [...(this.farMesh??[]),...this.mesh]){const depth=(tile.x-cameraX)*si-(tile.y-cameraY)*co,right=(tile.x-cameraX)*co+(tile.y-cameraY)*si;if(depth< -34||depth>(tile.far?3750:1450)||(tile.far&&depth<1400)||Math.abs(right)>depth*w/focal*.6+45)continue;visible.push({tile,d:depth});}visible.sort((a,b)=>b.d-a.d);
 for(const {tile,d} of visible){const fog=Math.min(.93,Math.max(0,d-100)/4000),rgb=tile.color.map((v,i)=>Math.round(v*(ambient*.86+(litSun?Math.max(0,tile.normal[0]*sunlight[0]+tile.normal[1]*sunlight[1]+sunlight[2])*.14:0))*(1-fog)+(night?[39,37,47]:light==='dusk'?[163,120,105]:light==='dawn'?[193,144,124]:[222,172,132])[i]*fog)),color=`rgb(${rgb})`,v=tile.corners.map(p=>project(...p));poly(v,color,color);}
 ...
 if(this.gravelKey!==this.chunkKey){this.gravelKey=this.chunkKey;this.gravel=[];const gx=Math.floor(s.x/20),gy=Math.floor(s.y/20);for(let j=-30;j<=30;j++)for(let i=-30;i<=30;i++){const x=gx+i,y=gy+j;this.gravel.push({x:x*20+noise(x,y)*18,y:y*20+noise(y,x)*18,r:.15+noise(x+71,y)*.55});}}
 ...
 minimap(canvas,s,course){const c=canvas.getContext('2d'),left=Math.max(0,Math.min(WORLD.width-1350,s.x-675)),top=Math.max(WORLD.minY,Math.min(WORLD.maxY-950,s.y-475));c.clearRect(0,0,216,152);c.fillStyle='#926243';c.fillRect(0,0,216,152);for(let y=0;y<152;y+=16)for(let x=0;x<216;x+=16){const z=Math.min(1,Math.max(0,ground(left+x/.16,top+y/.16)/230));c.fillStyle=`rgb(${Math.round(132+z*60)},${Math.round(87+z*53)},${Math.round(61+z*36)})`;c.fillRect(x,y,17,17);}c.save();c.scale(.16,.16);c.translate(-left,-top);c.fillStyle='#b68c64';for(const poly of [mesa,...(delta?.mesas??[])]){c.beginPath();poly.forEach((p,i)=>i?c.lineTo(...p):c.moveTo(...p));c.closePath();c.fill();}c.strokeStyle='#e1c28d55';c.lineWidth=18;c.beginPath();c.moveTo(960,470);c.lineTo(6200,490);c.stroke();samples.forEach((p,i)=>{c.fillStyle=s.collected.includes(i)?'#82b79d':'#f1d387';c.beginPath();c.arc(p.x,p.y,14,0,Math.PI*2);c.fill();});c.translate(s.x,s.y);c.rotate(s.heading);c.fillStyle='#e5fff0';c.beginPath();c.moveTo(0,-24);c.lineTo(-15,15);c.lineTo(15,15);c.closePath();c.fill();c.restore();c.fillStyle='#f0e7d2';c.font='10px Public';c.fillText('N ↑ · MAP',7,14);}
}
