...
function frame(t){const elapsed=(t-last)/1000||0,dt=Math.min(.05,elapsed);last=t;if(ready){if(!document.hidden)timeTour(expeditionRecord,elapsed,expedition?.active?'activities':paused?'menus':'driving');if(!paused){acc+=dt;while(acc>=1/60&&!paused){step(1/60);acc-=1/60;}if(t-lastDraw>=(quality==='performance'?32:0)){draw(Math.min(.05,(t-lastDraw)/1000));lastDraw=t;}if(t-lastSave>3000){save();lastSave=t;}}else if(dirty){draw(0);dirty=false;}
...
try{
 ...
 $('#welcome p').textContent=`CLASSIFIED · You are piloting a souped-up Perseverance clone. Retrace the recorded journey to restore its memory partition, and complete four roadside systems missions. Follow the chevrons; shortcuts and airtime are allowed. The 5× clone drive is equipped when you start the mission. Ingenuity is an optional discovery.`;
