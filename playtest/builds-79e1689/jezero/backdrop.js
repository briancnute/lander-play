...
import {upperSurface} from './belva.js';
...
export async function loadBackdrop(local,extension=null,upper=null){
 ...
 const baseJoined=joinedHeight;
 const upperJoined=(e,n)=>{
  if(!upper)return baseJoined(e,n);
  const ce=Math.max(-2700,Math.min(600,e)),cn=Math.max(-648,Math.min(2350,n));
  if(!upperSurface(ce,cn))return baseJoined(e,n);
  const p=toGame(ce,cn),t=Math.min(1,Math.hypot(e-ce,n-cn)/300);
  return baseJoined(e,n)+(upper.height(p.x,p.y)/4-2565-baseJoined(ce,cn))*(1-t);
 };
 const axis=Array.from({length:201},(_,i)=>-10000+i*100),northAxis=[...new Set([...axis,2350,-648])].sort((a,b)=>b-a);
 ...
 const backdrop=grid(axis,northAxis,upperJoined,(e,n)=>(!upper||!upperSurface(e,n))&&(extension?(e< -1500||e>3500||n< -2700||n>1500):Math.max(Math.abs(e),Math.abs(n))>1500));
 // Replace only the inaccessible Kodiak rectangle in the visual mesh.
 const visualIndices=[];for(let i=0;i<local.indices.length;i+=3){const ids=[local.indices[i],local.indices[i+1],local.indices[i+2]],e=ids.reduce((s,k)=>s+local.vertices[k*9]/HORIZONTAL_UNITS-1500,0)/3,n=ids.reduce((s,k)=>s+1500-local.vertices[k*9+1]/HORIZONTAL_UNITS,0)/3;if(!inKodiak(e,n)&&(!upper||!upperSurface(e,n)))visualIndices.push(...ids);}
 ...
 for(const edge of edges)for(let i=1;i<edge.length;i++){const p=toMetres((edge[i-1][0]+edge[i][0])/2,(edge[i-1][1]+edge[i][1])/2);if(upper&&upperSurface(p.east,p.north))continue;const a=[...edge[i-1],local.height(...edge[i-1])],b=[...edge[i],local.height(...edge[i])],c=[b[0],b[1],b[2]-160],d=[a[0],a[1],a[2]-160];triangle(skirt,a,b,c,[.65,.405,.265]);triangle(skirt,a,c,d,[.65,.405,.265]);}
