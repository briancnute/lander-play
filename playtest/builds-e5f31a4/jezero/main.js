...
const kindLabel=(kind,long=false)=>kind==='site'?(long?'JEZERO / VIEWPOINT':'VIEWPOINT'):kind==='history'?(long?'JEZERO / HISTORY STOP':'HISTORY STOP'):(long?'JEZERO / FACT':'FACT');
...
function resetAt(p=area.start,mode='free'){
 state=create(mode,area.course,selected);Object.assign(state,{x:p.x,y:p.y,heading:p.heading??-Math.PI*.35,z:area.ground(p.x,p.y),tune:mode==='trial'?raceKit(selected,true):explorationKit(selected,mods),collected:[...progress.collected],messageUntil:0});
...
const notesPrefs={view:'previews',sort:'recent',direction:'desc'};
try{const p=JSON.parse(localStorage.getItem(KEY+'.notes')||'null');if(p){if(['previews','details'].includes(p.view))notesPrefs.view=p.view;if(['name','recent'].includes(p.sort))notesPrefs.sort=p.sort;if(['asc','desc'].includes(p.direction))notesPrefs.direction=p.direction;}}catch{}
function rememberNotes(){try{localStorage.setItem(KEY+'.notes',JSON.stringify(notesPrefs));}catch{}}
...
function showCard(p,caller,kind){if(caller==='#notes-dialog')notesReturn=backTo;
 ...
 $('#discovery-map-note').textContent=p.locationNote??'Accessible discovery location';$('#discovery-map-button').setAttribute('aria-label',p.kind==='history'?'Open map for this accessible history stop; actual site is outside the playable map':'Open detailed discovery location map');
...
function renderNotes(){
 ...
 const rows=orderedNotes(discoveries,progress.collected,notesPrefs.sort,notesPrefs.direction);$('#notes').hidden=notesPrefs.view!=='previews';$('#notes-table').hidden=notesPrefs.view!=='details';
 for(const b of document.querySelectorAll('[data-notes-view]'))b.setAttribute('aria-pressed',String(b.dataset.notesView===notesPrefs.view));
 for(const h of document.querySelectorAll('[data-sort]')){const active=h.dataset.sort===notesPrefs.sort;h.setAttribute('aria-sort',active?(notesPrefs.direction==='asc'?'ascending':'descending'):'none');h.querySelector('span').textContent=active?(notesPrefs.direction==='asc'?'↑':'↓'):'';}
 $('#notes').replaceChildren(...rows.map(({site:p,known})=>{const a=document.createElement('article');a.className='journal-card';a.dataset.kind=p.kind;const text=document.createElement('div'),badge=document.createElement('small'),h=document.createElement('h3');badge.textContent=kindLabel(p.kind);h.textContent=known?p.name:'???';text.append(badge,h);
 if(known){const v=photos(p)[0],img=document.createElement('img');img.src=new URL(v.image,import.meta.url).href;img.alt=v.imageAlt;img.loading='lazy';a.append(img);const fact=document.createElement('p');fact.textContent=p.fact;const b=document.createElement('button');b.textContent='Open entry';b.onclick=()=>showCard(p,'#notes-dialog',p.kind);text.append(fact,b);}else{const hint=document.createElement('p');hint.textContent='Find this orb while exploring.';text.append(hint);}a.append(text);return a;}));
 $('#notes-table tbody').replaceChildren(...rows.map(({site:p,known})=>{const tr=document.createElement('tr'),name=document.createElement('td'),date=document.createElement('td');tr.dataset.kind=p.kind;if(known){const b=document.createElement('button');b.textContent=p.name;b.onclick=()=>showCard(p,'#notes-dialog',p.kind);name.append(b);date.textContent=photoDate(latestVisual(p));}else{name.textContent='???';date.textContent='—';}tr.append(name,date);return tr;}));
}
for(const b of document.querySelectorAll('[data-notes-view]'))b.onclick=()=>{notesPrefs.view=b.dataset.notesView;rememberNotes();renderNotes();};
for(const h of document.querySelectorAll('[data-sort]'))h.querySelector('button').onclick=()=>{const key=h.dataset.sort;notesPrefs.direction=key===notesPrefs.sort?(notesPrefs.direction==='asc'?'desc':'asc'):(key==='name'?'asc':'desc');notesPrefs.sort=key;rememberNotes();renderNotes();};
...
function drawMap(c=$('#map'),mini=false){
 ...
 for(const [i,p] of discoveries.entries()){const q=mapPoint(p,w,h);ctx.fillStyle=p.kind==='site'?'#a8d9df':p.kind==='history'?'#efc786':'#f4da9c';ctx.globalAlpha=mini?.55:1;if(p.kind==='history')ctx.fillRect(q.x-(mini?1:3),q.y-(mini?1:3),mini?2:6,mini?2:6);else{ctx.beginPath();ctx.arc(q.x,q.y,mini?.7:2.5,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=1;if(!mini&&nav.labels){ctx.font=`${w<260?8:10}px system-ui`;ctx.textAlign='center';const name=progress.collected.includes(i)?(p.mapName??p.name):'???';mapLabel(name,q,12);}}
...
$('#notes-back').onclick=closeSheet;
for(const b of document.querySelectorAll('[data-close]'))b.onclick=closeSheet;
for(const d of document.querySelectorAll('dialog'))d.addEventListener('cancel',e=>{e.preventDefault();if(d.id==='photo-dialog')d.close();else if(d.id==='result-dialog')explore();else closeSheet();});
async function roverMenu(){closeDialogs();setPause(true);try{await openCraft(selected,(id,chosenMods)=>{if(state.mode==='trial')endLap();Object.assign(mods,chosenMods??mods);selected=id;state.roverId=id;state.tune=explorationKit(id,mods);state.v=state.vz=state.boost=0;state.air=false;state.z=area.ground(state.x,state.y);previous=capturePose(state,state.z);gpu.reset(state);save();},{mods,jezero:true,onTab:menuTab});}catch{openDialog('#pause-dialog');notify('Rover selection could not load. Try again.');}}
async function chooseRace(){entryTime=0;entryArmed=false;closeDialogs();setPause(true);try{await openCraft(selected,id=>{raceExplorer??=selected;selected=id;startLap();},{race:true,jezero:true,onCancel:()=>{entryArmed=false;restoreExplorer();state.mode='free';state.tune=explorationKit(selected,mods);openDialog('#pause-dialog');}});}catch{openDialog('#pause-dialog');notify('Rover selection could not load. Try again.');}}
function menuTab(tab,caller=null){mapReturn=null;if(tab==='rover'){roverMenu();return;}openDialog(tab==='map'?'#map-dialog':tab==='notes'?'#notes-dialog':'#pause-dialog',tab==='notes'?(caller==='#map-dialog'?'#map-dialog':'#pause-dialog'):null);}
for(const nav of document.querySelectorAll('.menu-tabs')){const current=nav.closest('dialog').id;for(const [tab,text]of [['map','MAP'],['notes','FIELD NOTES'],['settings','SETTINGS'],['rover','ROVER']]){const b=document.createElement('button');b.textContent=text;b.dataset.tab=tab;if(tab==='notes')b.id=current==='pause-dialog'?'field-notes':current==='map-dialog'?'map-notes':'';if(tab==='rover'&&current==='pause-dialog')b.id='choose-craft';if(current===({map:'map-dialog',notes:'notes-dialog',settings:'pause-dialog'})[tab])b.setAttribute('aria-current','page');b.onclick=()=>menuTab(tab,'#'+current);nav.append(b);}}
$('#jump-to').onclick=()=>openDialog('#jump-dialog','#pause-dialog');
for(const [name,point]of [['Three Forks',()=>area.start],['Delta front',()=>discoveries[1]],['Crater floor',()=>toGame(750,-650)],['Kodiak overlook',()=>discoveries[3]]]){const b=document.createElement('button');b.textContent=name;b.onclick=()=>{resetAt({...point(),y:point().y-55,heading:Math.PI});explore();};$('#jump-sites').append(b);}
const raceJump=document.createElement('button');raceJump.textContent='Race · Delta circuit';raceJump.onclick=chooseRace;$('#jump-sites').append(raceJump);
