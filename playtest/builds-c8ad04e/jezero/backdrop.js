...
function grid(xs,ns,height,include=()=>true){
 ...
 for(let j=0;j<ns.length;j++)for(let i=0;i<xs.length;i++){const e=xs[i],n=ns[j],p=toGame(e,n),dx=height(e-2,n)-height(e+2,n),dy=height(e,n+2)-height(e,n-2),l=Math.hypot(dx,dy,4);vertices.set([p.x,p.y,(height(e,n)+2565)*4,dx/l,dy/l,4/l,.65,.405,.265],(j*xs.length+i)*9);}
...
export async function loadBackdrop(local){
 ...
 // No global height offset, invented mountain, distance compression or exaggeration.
 ...
 const visualIndices=[];for(let i=0;i<local.indices.length;i+=3){const ids=[local.indices[i],local.indices[i+1],local.indices[i+2]],e=ids.reduce((s,k)=>s+local.vertices[k*9]/4-1500,0)/3,n=ids.reduce((s,k)=>s+1500-local.vertices[k*9+1]/4,0)/3;if(!inKodiak(e,n))visualIndices.push(...ids);}
 // Edge skirts close sub-grid cracks between the coarse distant and local meshes.
 const skirt=[];const edges=[local.xs.map(x=>[x,0]),local.xs.map(x=>[x,12000]),local.ys.map(y=>[0,y]),local.ys.map(y=>[12000,y])];
