...
export function ground(x,y){
 ...
 return basinGround(x,y)*(1-blend)+z*blend;
}
...
export function update(s,input,dt,course){const tune=s.tune??TUNE;s.t+=dt;s.impact=Math.max(0,s.impact-dt*2);if(s.done){s.v*=Math.exp(-dt*6);return;}
 ...
 const edge=Math.min(s.x,3200-s.x,s.y,900-s.y);
 s.boundary=edge<40;
 ...
 if(s.turnaround){const error=angle(Math.atan2((s.x>1900?2640:s.x>1080?1600:540)-s.x,-((s.x>1080?470:380)-s.y))-s.heading);steer=clamp(error*2,-1,1);drive=true;brake=false;if(edge>65){s.turnaround=false;say(s,'Your controls',2);}}
 ...
 if(near>=0&&s.mode!=='trial'&&!s.air&&!s.turnaround){if(s.collecting!==near){s.collecting=near;s.collectTime=0;}s.collectTime+=dt;if(s.collectTime>=1.4){s.collected.push(near);s.collecting=-1;s.collectTime=0;say(s,samples[near].copy,6);if(s.collected.length===6){s.done=true;say(s,'All six samples secured.');}}}
