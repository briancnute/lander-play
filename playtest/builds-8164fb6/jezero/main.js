...
function showCard(p,caller,kind){if(caller==='#notes-dialog')notesReturn=backTo;
 ...
 $('.discovery-map-context').hidden=caller==='#notes-dialog';$('#discovery-map-note').textContent=p.locationNote??'Accessible discovery location';$('#discovery-map-button').setAttribute('aria-label',p.kind==='history'?'Open map for this accessible history stop; actual site is outside the playable map':'Open detailed discovery location map');
 const strip=$('#discovery-views');strip.replaceChildren();
 const views=photos(p);if(p.routeMap)views.push({image:'./assets/mission-route.svg',imageAlt:'Recorded rover route from the true landing site into Three Forks',imageDescription:'NASA drive vertices through sol 692. The circled landing site is outside the dashed playable boundary. This is a mission-history map, not a driving target.',imageCredit:'NASA Mars 2020 public map data · ASTRA map rendering',source:'https://science.nasa.gov/mission/mars-2020-perseverance/location-map/'});for(const [i,v]of views.entries()){const figure=document.createElement('figure'),button=document.createElement('button'),img=document.createElement('img'),caption=document.createElement('figcaption');button.type='button';button.setAttribute('aria-label','Open photograph '+(i+1)+' of '+p.name);img.src=new URL(v.image,import.meta.url).href;img.alt=v.imageAlt;if(!i)img.id='discovery-image';img.onerror=()=>{img.alt='Photograph unavailable — open for source details';};button.append(img);button.onclick=()=>inspectPhoto(p,v);caption.textContent=(v.iconic?'Iconic view · ':'')+photoDate(v.capturedAt);figure.append(button,caption);strip.append(figure);}
...
function setMapTarget(x,y,w,h){const target=mapTarget(x,y,w,h,area.bounds),old=nav.target;const same=old&&Math.hypot((old.x-target.x)/MAP_SIZE*w,(old.y-target.y)/MAP_SIZE*h)<12;nav.target=same||Math.hypot(target.x-state.x,target.y-state.y)<=CLOSE_RANGE?null:target;save();drawMap();drawMap($('#mini-map'),true);dirty=true;}
...
function renderNotes(){
 const total=regions.length+detailIndices.length,done=progress.regions.length+detailIndices.filter(i=>progress.collected.includes(i)).length;$('#notes-progress').textContent=`${done} / ${total} collected`;
