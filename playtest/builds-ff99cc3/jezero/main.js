...
import {loadJourney,restoreExpedition,ACTIVITY_IDS} from './expedition.js';
...
const RACE_VERSION='smooth-2_5x-v5';
...
let journey,expedition,expeditionRecord,activitySave={},basemap='satellite';
...
function readSave(){try{const s=JSON.parse(localStorage.getItem(KEY)||'null');if(!s||s.version!==SITE_VERSION)return;
 ...
 expeditionRecord=restoreExpedition(s.expedition,journey);activitySave=s.activityState&&typeof s.activityState==='object'?s.activityState:{};basemap=s.basemap==='relief'?'relief':'satellite';
 ...
 if(s.navigation?.target){const t=s.navigation.target;if(Number.isFinite(t.x)&&Number.isFinite(t.y)&&t.x>=mapBounds.minX&&t.x<=mapBounds.width&&t.y>=mapBounds.minY&&t.y<=mapBounds.maxY)nav.target=reachableTarget(t,area.bounds);}
...
function showCard(p,caller,kind){if(caller==='#notes-dialog')notesReturn=backTo;
 ...
 $('.discovery-map-context').hidden=caller==='#notes-dialog';$('#discovery-map-note').textContent=p.locationNote??'Accessible discovery location';$('#discovery-map-button').setAttribute('aria-label',p.id==='landing'?'Open map for the Three Forks history stop, not the actual touchdown point':'Open detailed discovery location map');
...
function setMapTarget(x,y,w,h){const target=mapTarget(x,y,w,h,area.bounds),old=nav.target;const same=old&&Math.hypot(old.x-target.x,old.y-target.y)<Math.min(120,12/mapFrame(w,h).scale);nav.target=same||Math.hypot(target.x-state.x,target.y-state.y)<=CLOSE_RANGE?null:target;save();drawMap();drawMap($('#mini-map'),true);dirty=true;}
...
function renderNotes(){
 syncFloor();
 const found=detailIndices.filter(i=>progress.collected.includes(i)).length+regions.filter(r=>!r.ambient&&progress.regions.includes(r.regionId??r.id)).length,total=detailIndices.length+regions.filter(r=>!r.ambient).length;$('#notes-progress').textContent=`${found} / ${total} discoveries · All built terrain is open · Delta circuit: ${progress.completedActivities.includes('delta-trial')?'complete':'optional'}`;
 const noteRow=(p,known)=>{const entry=document.createElement('div'),row=document.createElement('button');entry.className='note-entry';entry.style.setProperty('--site-color',siteColor(p));row.className='note-row';row.dataset.kind=p.kind;const img=document.createElement('img'),type=document.createElement('small'),name=document.createElement('span');type.textContent=kindLabel(p.kind);name.textContent=known?p.name:'???';if(known){const v=photos(p)[0];img.src=new URL(v.image,import.meta.url).href;img.alt='';row.onclick=()=>showCard(p,'#notes-dialog',p.kind);}else{img.hidden=true;row.disabled=true;}row.append(img,type,name);entry.append(row);if(known){const actions=document.createElement('div');actions.className='note-actions';const target=document.createElement('button'),jump=document.createElement('button');target.textContent='Target on map';target.onclick=()=>targetSite(p);jump.textContent='Jump to site';jump.onclick=()=>requestJump(p,'#notes-dialog');actions.append(target,jump);entry.append(actions);}return entry;};
 $('#notes').replaceChildren(...regions.filter(r=>!r.ambient||detailIndices.some(i=>regionIdForDetail(i)===(r.regionId??r.id))).map(region=>{const id=region.regionId??region.id,known=region.ambient||progress.regions.includes(id),section=document.createElement('section');section.className='note-region';const head=document.createElement('div');head.className='note-region-head';const title=document.createElement('h3');title.textContent=known?region.name:'???';const count=document.createElement('small'),children=detailIndices.filter(i=>regionIdForDetail(i)===id);count.textContent=known?(children.length?`${children.filter(i=>progress.collected.includes(i)).length} / ${children.length} sites`:'REGION OVERVIEW'):'REGION';head.append(title,count);section.append(head);if(!region.ambient)section.append(noteRow(region,known));if(known)for(const i of children)section.append(noteRow(discoveries[i],progress.collected.includes(i)));return section;}));
 const archived=retiredDetailIndices.filter(i=>progress.collected.includes(i)),overviews=regions.filter(r=>r.ambient&&!r.groupOnly&&progress.regions.includes(r.regionId??r.id));if(archived.length||overviews.length){const group=document.createElement('details'),label=document.createElement('summary');label.textContent='Earlier field notes';group.append(label,...overviews.map(r=>noteRow(r,true)),...archived.map(i=>noteRow(discoveries[i],true)));$('#notes').append(group);}
}
...
function drawMap(c=$('#map'),mini=false){
 ...
 if(journey&&state.mode!=='trial'){
  ...
  for(const [name,p,color]of [['START',journey.start,'#b7efc3'],['FINISH',journey.finish,'#fff0ba']]){const q=mapPoint(p,w,h,bounds);ctx.fillStyle=color;ctx.fillRect(q.x-3,q.y-3,6,6);if(!mini){ctx.font='bold 10px system-ui';ctx.textAlign='center';ctx.fillText(name,q.x,q.y+17);}}
  if(expeditionRecord?.status==='active'){const p=journey.checkpoints[expeditionRecord.next]??journey.finish,q=mapPoint(p,w,h,bounds);ctx.strokeStyle='#9be3ed';ctx.lineWidth=2;ctx.beginPath();ctx.arc(q.x,q.y,6,0,Math.PI*2);ctx.stroke();}ctx.restore();
 }
 ...
 if(!mini&&nav.labels&&state.mode!=='trial'){ctx.font='10px system-ui';ctx.textAlign='center';ctx.fillStyle='#f5ead1';for(const p of worldLocations)mapLabel(p.name,mapPoint(p,w,h,bounds),-12);}
 ...
 for(const p of regions.filter(r=>!r.ambient)){const id=p.regionId??p.id,q=mapPoint(p,w,h,bounds),known=progress.regions.includes(id);if(q.x<-12||q.x>w+12||q.y<-12||q.y>h+12)continue;ctx.fillStyle=known?'#d5edc1':'#efe2bc';ctx.beginPath();ctx.arc(q.x,q.y,mini?3.4:5,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#17322e';ctx.lineWidth=1;ctx.stroke();ctx.font=`${mini?8:10}px system-ui`;ctx.fillStyle='#f5ead1';mapLabel(known?p.name:'???',q,mini?12:15);}
 for(const i of detailIndices){const p=discoveries[i],id=regionIdForDetail(i);if(!detailAvailable(i,progress.regions))continue;const q=mapPoint(p,w,h,bounds);if(q.x<-8||q.x>w+8||q.y<-8||q.y>h+8)continue;ctx.fillStyle=siteColor(p);ctx.globalAlpha=mini?.9:1;if(p.kind==='history')ctx.fillRect(q.x-(mini?3:4),q.y-(mini?3:4),mini?6:8,mini?6:8);else{ctx.beginPath();ctx.arc(q.x,q.y,mini?2.8:4,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=1;if(!mini&&nav.labels){ctx.font=`${w<260?8:10}px system-ui`;ctx.textAlign='center';mapLabel(progress.collected.includes(i)?(p.mapName??p.name):'???',q,12);}}
 const rover=mapPoint(state,w,h,bounds);ctx.save();ctx.translate(rover.x,rover.y);ctx.rotate(state.heading);ctx.fillStyle='#e2ffe3';ctx.strokeStyle='#16332d';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(0,-6);ctx.lineTo(4,4);ctx.lineTo(0,2);ctx.lineTo(-4,4);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();
...
function step(dt){previous=capturePose(state,area.ground(state.x,state.y));driveStep(state,input,dt,area.course,{...area.driveArea,roverHandling:true,airControl:state.tune.boostKit,boostKit:state.tune.boostKit});if(stormsEnabled&&!expedition?.protectsArt)storm.tick(dt,state,()=>gpu.tracks.reset());expedition?.tick(dt,previous);
...
$('#explore').onclick=()=>expedition.start();$('#free-explore').onclick=explore;$('#race-again').onclick=chooseRace;$('#result-explore').onclick=()=>expedition?.active?.id==='delta-trial'?expedition.returnToRoute():explore();
...
async function roverMenu(){closeDialogs();setPause(true);try{await openCraft(selected,(id,chosenMods)=>{if(state.mode==='trial')endLap();Object.assign(mods,chosenMods??mods);selected=id;state.roverId=id;state.tune=explorationKit(id,mods);state.v=state.vz=state.boost=0;state.air=false;state.z=area.ground(state.x,state.y);previous=capturePose(state,state.z);gpu.reset(state);save();},{mods,jezero:true,compact:true,jumpAvailable:progress.regions.length>0||progress.collected.length>0,onTab:menuTab});}catch{openDialog('#pause-dialog');notify('Rover selection could not load. Try again.');}}
async function chooseRace(){entryTime=0;entryArmed=false;closeDialogs();setPause(true);try{await openCraft(selected,id=>{raceExplorer??=selected;selected=id;startLap();},{race:true,jezero:true,compact:true,onCancel:()=>{entryArmed=false;restoreExplorer();state.mode='free';state.tune=explorationKit(selected,mods);openDialog('#pause-dialog');}});}catch{openDialog('#pause-dialog');notify('Rover selection could not load. Try again.');}}
function menuTab(tab,caller=null){mapReturn=null;if(tab==='rover'){roverMenu();return;}const target={map:'#map-dialog',notes:'#notes-dialog',jump:'#jump-dialog',settings:'#pause-dialog'}[tab]??'#pause-dialog';openDialog(target,tab==='notes'?(caller==='#map-dialog'?'#map-dialog':'#pause-dialog'):null);}
function syncMenuTabs(){const available=progress.regions.length>0||progress.collected.length>0;for(const b of document.querySelectorAll('.menu-tabs [data-tab="jump"]')){b.disabled=!available;b.title=available?'':'Visit a location to unlock Jump To';}}
for(const tabs of document.querySelectorAll('.menu-tabs')){const current=tabs.closest('dialog').id;for(const [tab,label]of [['map','MAP'],['notes','FIELD NOTES'],['jump','JUMP TO'],['settings','SETTINGS'],['rover','ROVER']]){const b=document.createElement('button');b.textContent=label;b.dataset.tab=tab;if(tab==='notes')b.id=current==='pause-dialog'?'field-notes':current==='map-dialog'?'map-notes':'';if(tab==='rover'&&current==='pause-dialog')b.id='choose-craft';if(current===({map:'map-dialog',notes:'notes-dialog',jump:'jump-dialog',settings:'pause-dialog'})[tab])b.setAttribute('aria-current','page');b.onclick=()=>menuTab(tab,'#'+current);tabs.append(b);}}
function targetSite(p){nav.target=reachableTarget(p,area.bounds);nav.labels=true;save();dirty=true;openDialog('#map-dialog','#notes-dialog');notify(`Target set · ${p.name}`,3);}
function requestJump(p,returnTo='#jump-dialog'){pendingJump={p,returnTo};closeDialogs();setPause(true);$('#jump-confirm-title').textContent=`Jump to ${p.name}?`;$('#jump-confirm-copy').textContent='Leave your current position and jump to this visited site?';$('#jump-confirm-dialog').showModal();}
...
$('#jump-confirm-go').onclick=()=>{const p=pendingJump?.p;if(!p)return;pendingJump=null;$('#jump-confirm-dialog').close();resetAt({...p,y:p.y-55,heading:Math.PI});explore();};
function renderJumpSites(){const list=$('#jump-sites');list.replaceChildren();const add=(p,prefix='')=>{const b=document.createElement('button');b.textContent=prefix+p.name;b.onclick=()=>requestJump(p);list.append(b);};for(const region of regions){const id=region.regionId??region.id;if(!region.groupOnly&&progress.regions.includes(id))add(region);}for(const i of detailIndices)if(progress.collected.includes(i))add(discoveries[i],'SITE · ');$('#jump-empty').hidden=!!list.children.length;syncMenuTabs();}
...
try{
 area=await loadArea();journey=await loadJourney();area.world.segments=journey.segments;expeditionRecord=restoreExpedition(null,journey);syncFloor();readSave();gpu=new GPURenderer($('#world'),area);gpu.shadowMode='none';gpu.softSurfaceLighting=true;gpu.flightLighting=1;posts=gateGeometry();storm.southBoundary=area.bounds.maxY+1600;resetAt(progress.position??{...journey.start,heading:-.4});for(const i of progress.collected)if(discoveries[i]&&Math.hypot(state.x-discoveries[i].x,state.y-discoveries[i].y)<CLOSE_RANGE)observedInside.add(i);for(const p of regions){const id=p.regionId??p.id;if(progress.regions.includes(id)&&Math.hypot(state.x-p.x,state.y-p.y)<CLOSE_RANGE)observedInside.add('region:'+id);}entryArmed=Math.hypot(state.x-raceSite.x,state.y-raceSite.y)>=raceSite.radius;ready=true;minimap.layout();$('#map-enabled').checked=minimap.state.enabled;$('#storms-enabled').checked=stormsEnabled;$('#quality').value=quality;$('#light').value=light;for(const b of document.querySelectorAll('[data-light]'))b.setAttribute('aria-pressed',String(b.dataset.light===light));$('#loading').hidden=true;
 expedition=createActivities({area,gpu,storm,nav,get state(){return state},reset:resetAt,explore,resume:()=>setPause(false),pause:()=>setPause(true),openDialog,closeDialogs,save,draw,endLap,chooseRace,travelSetup:()=>({rover:selected,kit:mods.kit}),restoreTravel:p=>{if(KITS[p.rover])selected=p.rover;if(typeof p.kit==='boolean')mods.kit=p.kit;},enableKit:()=>{selected='perseverance';mods.kit=true;}},journey,expeditionRecord,activitySave);
 ...
 window.__jezero={get state(){return state},area,gpu,storm,progress,expedition,get worldUnlocked(){return true},get expansionComplete(){return area.extension.unlocked&&detailIndices.every(i=>progress.collected.includes(i))&&regions.filter(r=>!r.ambient).every(r=>progress.regions.includes(r.regionId??r.id))},get paused(){return paused},get input(){return input},landmarks,regions,nav,raceSite,get entryTime(){return entryTime},mods,minimap,get stormsEnabled(){return stormsEnabled},resetAt,step,draw,startLap,chooseRace,explore,setPause,save,openDialog,renderNotes,drawMap};requestAnimationFrame(frame);
