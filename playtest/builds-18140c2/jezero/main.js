...
function readSave(){try{const s=JSON.parse(localStorage.getItem(KEY)||'null');if(!s||s.version!==SITE_VERSION)return;
 ...
 progress.legacyBests={...s.legacyBests,...(s.raceVersion==='coast-v2'?{}:Object.fromEntries(Object.entries(s.bests??{}).map(([id,time])=>[(s.raceVersion??'original')+':'+id,time])))};
 if(s.raceVersion==='coast-v2'&&s.bests&&typeof s.bests==='object')for(const id of Object.keys(KITS))if(Number.isFinite(s.bests[id])&&s.bests[id]>0)progress.bests[id]=s.bests[id];
...
function save(){if(!ready)return;const position=state.mode==='free'&&!state.air&&!state.turnaround?{x:state.x,y:state.y,heading:state.heading}:progress.position;
 try{localStorage.setItem(KEY,JSON.stringify({version:SITE_VERSION,siteCatalog:2,collected:progress.collected,bests:progress.bests,legacyBests:progress.legacyBests,raceVersion:'coast-v2',rover:raceExplorer??selected,position,quality,light,seenLandmarks:progress.seenLandmarks,navigation:nav,mods,stormsEnabled,minimap:minimap.state}));progress.position=position;}catch{if(!saveWarned){saveWarned=true;notify('Progress cannot be saved in this browser.');}}}
...
function renderNotes(){
 ...
 $('#notes').replaceChildren(...rows.map(({site:p,known})=>{const a=document.createElement('article');a.className='journal-card';a.dataset.kind=p.kind;const text=document.createElement('div'),badge=document.createElement('small'),h=document.createElement('h3');badge.textContent=p.kind==='site'?'VIEWPOINT':'FACT';h.textContent=known?p.name:'???';text.append(badge,h);
