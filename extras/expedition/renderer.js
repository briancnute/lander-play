...
export class Renderer {
 ...
 if(!this.gravel){this.gravel=[];let seed=47;const rnd=()=>((seed=(seed*1664525+1013904223)>>>0)/4294967296);for(let i=0;i<11000;i++){const x=rnd()*3900-300,y=rnd()*1360-300;this.gravel.push({x,y,r:.15+rnd()*.55});}}
 ...
 minimap(canvas,s,course){const c=canvas.getContext('2d'),left=Math.max(0,Math.min(1850,s.x-675));c.clearRect(0,0,216,152);c.fillStyle='#926243';c.fillRect(0,0,216,152);c.save();c.scale(.16,.16);c.translate(-left,0);c.fillStyle='#b68c64';for(const poly of [mesa,...(delta?.mesas??[])]){c.beginPath();poly.forEach((p,i)=>i?c.lineTo(...p):c.moveTo(...p));c.closePath();c.fill();}c.strokeStyle='#e1c28d55';c.lineWidth=18;c.beginPath();c.moveTo(960,470);c.lineTo(2150,490);c.stroke();samples.forEach((p,i)=>{c.fillStyle=s.collected.includes(i)?'#82b79d':'#f1d387';c.beginPath();c.arc(p.x,p.y,14,0,Math.PI*2);c.fill();});c.translate(s.x,s.y);c.rotate(s.heading);c.fillStyle='#e5fff0';c.beginPath();c.moveTo(0,-24);c.lineTo(-15,15);c.lineTo(15,15);c.closePath();c.fill();c.restore();c.fillStyle='#f0e7d2';c.font='10px Public';c.fillText('N ↑ · MAP',7,14);}
}
