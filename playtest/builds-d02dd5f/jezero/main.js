import {mapBounds,geology,raceSite,mapTarget,advanceEntry,CLOSE_RANGE,regionAt} from './navigation.js';
...
const nav={target:null};let entryTime=0,entryArmed=true;const observedInside=new Set();
...
function readSave(){try{const s=JSON.parse(localStorage.getItem(KEY)||'null');if(!s||s.version!==SITE_VERSION)return;
 ...
 if(s.navigation?.target){const t=s.navigation.target;if(Number.isFinite(t.x)&&Number.isFinite(t.y)&&t.x>=0&&t.x<=12000&&t.y>=0&&t.y<=12000)nav.target={x:t.x,y:t.y};}
...
function showCard(p,caller,kind){if(caller==='#notes-dialog')notesReturn=backTo;
 $('#discovery-kind').textContent=kind==='site'?'JEZERO / SITE':'JEZERO / FACT';
...
function setMapTarget(x,y,w,h){const target=mapTarget(x,y,w,h),old=nav.target;const same=old&&Math.hypot((old.x-target.x)/12000*w,(old.y-target.y)/12000*h)<12;nav.target=same||Math.hypot(target.x-state.x,target.y-state.y)<=CLOSE_RANGE?null:target;save();drawMap();drawMap($('#mini-map'),true);dirty=true;}
...
function renderNotes(){
 ...
 $('#notes').replaceChildren(...discoveries.map((p,i)=>{const known=progress.collected.includes(i),read=known||(p.kind==='site'&&progress.seenLandmarks.includes(p.id));const a=document.createElement('article');a.className='journal-card';const badge=document.createElement('small');badge.textContent=p.kind==='site'?'SITE':'FACT';const h=document.createElement('h3');h.textContent=read?p.name:'?';a.append(badge,h);
...
function drawMap(c=$('#map'),mini=false){
 ...
 c.dataset.geologyVisible=String(!mini);c.dataset.sitesVisible=String(!mini);c.dataset.raceVisible=String(!mini&&state.mode==='trial');c.dataset.targetVisible=String(!!nav.target);
 if(!mini){
  ...
  ctx.textAlign='center';ctx.font=`${w<260?9:11}px system-ui`;ctx.fillStyle='#f1e4c9';for(const p of geology){const q=mapPoint(p,w,h),tw=ctx.measureText(p.name).width,x=Math.max(tw/2+4,Math.min(w-tw/2-4,q.x));ctx.fillText(p.name,x,q.y-10);}
  for(const p of discoveries){const q=mapPoint(p,w,h);ctx.fillStyle='#f4da9c';ctx.beginPath();ctx.arc(q.x,q.y,2.5,0,Math.PI*2);ctx.fill();}
...
$('#pause-button').onclick=()=>openDialog('#pause-dialog');$('#mini-map-button').onclick=()=>openDialog('#map-dialog');$('#resume').onclick=()=>{closeDialogs();exploreOrResume();};
