...
function save(){if(!ready)return;const position=state.mode==='free'&&!state.air&&!state.turnaround?{x:state.x,y:state.y,heading:state.heading}:progress.position;
 try{localStorage.setItem(KEY,JSON.stringify({version:SITE_VERSION,collected:progress.collected,bests:progress.bests,legacyBests:progress.legacyBests,raceVersion:'coast-v2',rover:raceExplorer??selected,position,quality,light,seenLandmarks:progress.seenLandmarks,navigation:nav,mods,stormsEnabled,minimap:minimap.state}));progress.position=position;}catch{if(!saveWarned){saveWarned=true;notify('Progress cannot be saved in this browser.');}}}
...
function showCard(p,caller,kind){if(caller==='#notes-dialog')notesReturn=backTo;
 ...
 const views=photos(p);for(const [i,v]of views.entries()){const figure=document.createElement('figure'),button=document.createElement('button'),img=document.createElement('img'),caption=document.createElement('figcaption');button.type='button';button.setAttribute('aria-label','Open photograph '+(i+1)+' of '+p.name);img.src=new URL(v.image,import.meta.url).href;img.alt=v.imageAlt;if(!i)img.id='discovery-image';img.onerror=()=>{img.alt='Photograph unavailable — open for source details';};button.append(img);button.onclick=()=>inspectPhoto(p,v);caption.textContent=(v.iconic?'Iconic view · ':'')+photoDate(v.capturedAt);figure.append(button,caption);strip.append(figure);}
