...
function setMapTarget(x,y,w,h){const target=mapTarget(x,y,w,h,area.bounds),old=nav.target;const same=old&&Math.hypot(old.x-target.x,old.y-target.y)*mapFrame(w,h).scale<12;nav.target=same||Math.hypot(target.x-state.x,target.y-state.y)<=CLOSE_RANGE?null:target;save();drawMap();drawMap($('#mini-map'),true);dirty=true;}
