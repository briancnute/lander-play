...
import {mapBounds,raceSite,mapTarget,advanceEntry,CLOSE_RANGE,regionAt,reachableTarget} from './navigation.js';
...
function renderNotes(){
 syncFloor();
 const found=detailIndices.filter(i=>progress.collected.includes(i)).length+regions.filter(r=>!r.ambient&&progress.regions.includes(r.regionId??r.id)).length;$('#notes-progress').textContent=`${found} / 5 discoveries · All built terrain is open · Delta circuit: ${progress.completedActivities.includes('delta-trial')?'complete':'optional'}`;
