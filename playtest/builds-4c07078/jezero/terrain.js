...
export const toGame=(east,north)=>({x:(east+1500)*4,y:(1500-north)*4});
export const toMetres=(x,y)=>({east:x/4-1500,north:1500-y/4});
...
export async function loadTerrain(){
 ...
 const xs=info.eastMetres.map(e=>(e+1500)*4),ys=info.northMetres.map(n=>(1500-n)*4);
 ...
 for(let j=0;j<ys.length;j++)for(let i=0;i<xs.length;i++){
  const k=j*xs.length+i,x=xs[i],y=ys[j],e=x/4-1500,n=1500-y/4;
...
export function buildRocks(height,route,samples){
 ...
 for(let i=0;i<2100;i++){
  const x=3400+random()*6500,y=4200+random()*5600,r=.7+random()**4*7;
  if(nearPath(x,y)||(Math.abs(x-8400)<65&&Math.abs(y-8800)<650)||Math.hypot(x-7400,y-7980)<180)continue;
