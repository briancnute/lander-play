...
import {kit,KITS,explorationKit} from '../mars-renderer/kits.js';
...
const mods={speed:false,air:false};
...
function readSave(){try{const s=JSON.parse(localStorage.getItem(KEY)||'null');if(!s||s.version!==SITE_VERSION)return;
 ...
 mods.speed=s.mods?.speed===true;mods.air=s.mods?.air===true;nav.labels=s.navigation?.labels===true;
...
function resetAt(p=area.start,mode='free'){
 state=create(mode,area.course,selected);Object.assign(state,{x:p.x,y:p.y,heading:p.heading??-Math.PI*.35,z:area.ground(p.x,p.y),tune:mode==='trial'?kit(selected):explorationKit(selected,mods),collected:[...progress.collected],messageUntil:0});
...
function endLap(){state.mode='free';state.tune=explorationKit(selected,mods);state.done=false;state.countdown=0;state.time=0;notify('Explore freely');updateHud();save();}
function finishLap(){const time=state.time,old=progress.bests[selected];if(!old||time<old)progress.bests[selected]=time;
 ...
 state.mode='free';state.tune=explorationKit(selected,mods);state.done=false;state.countdown=0;state.v=0;openDialog('#result-dialog');updateHud();save();}
let currentRegion='';
...
function step(dt){previous=capturePose(state,area.ground(state.x,state.y));driveStep(state,input,dt,area.course,{...area.driveArea,airControl:state.mode==='free'&&mods.air});storm.tick(dt,state,()=>gpu.tracks.reset());
...
async function roverMenu(){closeDialogs();setPause(true);try{await openCraft(selected,(id,chosenMods)=>{if(state.mode==='trial')endLap();Object.assign(mods,chosenMods??mods);selected=id;state.roverId=id;state.tune=explorationKit(id,mods);state.v=state.vz=state.boost=0;state.air=false;state.z=area.ground(state.x,state.y);previous=capturePose(state,state.z);gpu.reset(state);save();},{mods,onTab:menuTab});}catch{openDialog('#pause-dialog');notify('Rover selection could not load. Try again.');}}
...
for(const [name,point]of [['Three Forks',()=>area.start],['Delta front',()=>discoveries[1]],['Crater floor',()=>raceSite],['Kodiak overlook',()=>discoveries[3]]]){const b=document.createElement('button');b.textContent=name;b.onclick=()=>{resetAt({...point(),y:point().y-55,heading:Math.PI});explore();};$('#jump-sites').append(b);}
...
$('#light').onchange=e=>{light=e.target.value;lighting();for(const b of document.querySelectorAll('[data-light]'))b.setAttribute('aria-pressed',String(b.dataset.light===light));dirty=true;save();};$('#quality').onchange=e=>{quality=e.target.value;dirty=true;save();};$('#storm').onclick=()=>{storm.start(state);closeDialogs();exploreOrResume();};
