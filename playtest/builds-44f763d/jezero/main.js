...
function setPause(value){paused=value;if(value)landmarkLayer.hide();clearInput();acc=0;if(state)previous=capturePose(state,area.ground(state.x,state.y));$('#lamps').hidden=true;dirty=true;if(value)save();else if(document.activeElement instanceof HTMLElement)document.activeElement.blur();}
...
function showCard(p,caller,kind){if(caller==='#notes-dialog')notesReturn=backTo;
 ...
 const img=$('#discovery-image');img.hidden=false;img.src=new URL(p.image,import.meta.url).href;img.alt=p.imageAlt;img.onerror=()=>{img.hidden=true;};
 $('#discovery-credit').textContent=p.imageCredit+(kind==='landmark'?' · Real landmark; reference camera differs from this view.':' · Regional reference; not a matched view from this stop.');
 $('#discovery-source').href=p.source;openDialog('#discovery-dialog',caller);
...
function drawMap(c=$('#map'),mini=false){
 ...
 ctx.font=mini?'9px system-ui':'12px system-ui';const shortcut=mapPoint(toGame(350,-495),w,h);ctx.fillStyle='#e1d6b6';if(!mini&&racing)ctx.fillText('Shortcut',shortcut.x,shortcut.y-13);const jump=mapPoint(area.jump,w,h);ctx.fillStyle='#d5d7c1';ctx.fillText(mini?'↟':'↟ Jump',jump.x,jump.y+4);
 ...
 if(!mini){$('#race-map-key').hidden=!racing;$('#best-lap').hidden=!racing;}
...
function draw(dt){gpu.quality=quality;gpu.storm=storm;const shown=paused?state:displayState(previous,state,acc*60,area.ground(state.x,state.y));gpu.draw(shown,dt,$('#labels'));sky.draw(gpu,shown);wayfinding();placeLamps();landmarkLayer.draw(gpu,area,state,landmarkLabels&&!paused&&!welcome,[...document.querySelectorAll('header,.controls,#mini-map-button,#race-hud,#toast,#lamps')].filter(e=>!e.hidden).map(e=>e.getBoundingClientRect()));}
function step(dt){previous=capturePose(state,area.ground(state.x,state.y));const previousCollected=[...state.collected],before=state.collected.length;driveStep(state,input,dt,area.course,area);storm.tick(dt,state,()=>gpu.tracks.reset());
 if(state.collected.length!==before){progress.collected=[...state.collected];save();updateHud();const found=state.collected.find(i=>!previousCollected.includes(i));if(found!==undefined)showDiscovery(found);}
...
try{
 ...
 window.__jezero={get state(){return state},area,gpu,storm,progress,get paused(){return paused},get input(){return input},landmarks,get landmarkLabels(){return landmarkLabels},resetAt,step,draw,startLap,explore,setPause,save,openDialog};requestAnimationFrame(frame);
